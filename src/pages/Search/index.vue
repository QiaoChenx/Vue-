...
export default {
    ...
    mounted() {
        this.$bus.$on("handleTrademark", (trademark) => {
    ...
    computed: {
        ...
        oneAsc() {
            return this.searchParams.order == "1:asc";
        },

        oneDesc() {
            return this.searchParams.order == "1:desc";
        },
    ...
    methods: {
        ...
        handleProps(props, attrValue) {
            let isRepeat = this.searchParams.props.every((item) => {
                return item.split(":")[1] !== attrValue;
            });
            if (isRepeat) {
                this.searchParams.props.push(
                    props.attrId + ":" + attrValue + ":" + props.attrName
                );
            }
        },
        ...
        changeOrder(n) {
            ...
            if (order.split(":")[0] == n) {
                this.searchParams.order =
                    order.split(":")[1] == "asc" ? n + ":desc" : n + ":asc";
            } else {
                this.searchParams.order = n + ":desc";
            }
        },
